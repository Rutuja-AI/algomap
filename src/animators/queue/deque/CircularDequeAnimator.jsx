...
/**
 * 🔄 Circular Deque Visualizer — Synced with CircularQueue Style
 * ---------------------------------------------------------------
 * ✅ Same layout & toggle as CircularQueue
 * ✅ Works with deque_state or buffer
 * ✅ Shows nodes correctly (Array ↔ Ring)
 * ✅ Narration + Legend + Smooth transitions
 */
export default function CircularDequeAnimator({
  steps = [],
  currStep = 0,
  meta = {},
  viewMode = "array",
  ...
  useEffect(() => {
    ...
    window.addEventListener("deque-view-toggle", handler);
    return () => window.removeEventListener("deque-view-toggle", handler);
  }, []);

  // 🧮 Rebuild deque state
  const { buffer, front, rear, narration } = useMemo(() => {
    let cap = meta?.capacity || steps[0]?.meta?.capacity || 5;
    let buf = Array(cap).fill("∅");
    let f = -1,
      r = -1,
      narr = "";
    ...
    for (let i = 0; i <= currStep && i < steps.length; i++) {
      ...
      const state =
        Array.isArray(s.deque_state) && s.deque_state.length
          ? s.deque_state
          : Array.isArray(s.buffer) && s.buffer.length
          ? s.buffer
          : null;

      if (state) buf = [...state].map(normalizeVal);

      if (typeof s.front === "number") f = s.front;
      if (typeof s.rear === "number") r = s.rear;
    }
  ...
  const toggleView = () => {
    ...
    setMode(next);
    meta.viewMode = next;
    window.dispatchEvent(new CustomEvent("deque-view-toggle", { detail: next }));
  };
    ...
    <div className="w-full h-full flex flex-col items-center gap-4 justify-center">
      ...
      <div className="flex items-center gap-3">
        <h2 className="text-lg font-bold text-gray-700">
          Circular Deque ({mode === "array" ? "Array View" : "Ring View"})
        </h2>
        <button
          onClick={toggleView}
      ...
      <div className="text-base opacity-80 text-center italic mt-2">
        🗣️ {narration || "circular deque"}
      </div>

      {/* legend */}
      <div className="text-xs opacity-70 flex gap-4 mt-2">
        <span className="text-blue-600 font-bold">FRONT</span> = dequeue/enqueue left
        <span className="text-rose-600 font-bold">REAR</span> = dequeue/enqueue right
